var dayView = function(container,activitiesContr){
	...
	this.addDay = function(){
		...
		var dayn = this.days.length - 1;
		var startT = container.find("#day"+dayn+" .startTime");
		startT.change(function(){
			model.days[dayn].setStart(parseInt(startT.val().substr(0,startT.val().indexOf(':'))),parseInt(startT.val().substr(startT.val().indexOf(':')+1, startT.val().length + 1)));
		});
	...
	this.update = function(arg){
		...
		for(day = 0; day< model.days.length; day++){
			...
			this.days[day].find(".startTime").val(model.days[day].getStart());
			...
			for(i = 0; i< model.days[day]._activities.length; i++){
				...
				activitiesContr.addListenersForActivity(activity,i,day);
